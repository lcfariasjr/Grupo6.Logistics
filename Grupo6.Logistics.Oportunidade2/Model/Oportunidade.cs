...
namespace Grupo6.Logistics.Oportunidade2.Model
{
    public class Oportunidade
    {
        ...
        public string GetNameListPriceById(Guid ListaPrecoId)
        {
            QueryExpression busca = new QueryExpression("pricelevel");
            busca.ColumnSet.AddColumn("name");
            busca.Criteria.AddCondition("pricelevelid", ConditionOperator.Equal, ListaPrecoId);
            EntityCollection listas = Service.RetrieveMultiple(busca);
            return listas.Entities.FirstOrDefault().Attributes["name"].ToString();
        }
        public Guid GetListPriceIdByName(string NomeLista)
        {
            QueryExpression busca = new QueryExpression("pricelevel");
            busca.Criteria.AddCondition("name", ConditionOperator.Equal, NomeLista);
            EntityCollection listas = Service.RetrieveMultiple(busca);
            return listas.Entities.FirstOrDefault().Id;
        }
    }
}
